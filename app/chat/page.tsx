          ...
          <div className="flex w-full items-end rounded bg-gray-500 px-3 py-2 text-lg text-gray-200">
            <textarea
              ...
              onKeyDown={async (e) => {
                ...
                isTyping.mutate({ typing: true, userId });
              }}
              onKeyUp={(e) => {
                if (e.key === 'Shift') {
              ...
              onBlur={() => {
                setEnterToPostMessage(true);
                isTyping.mutate({ typing: false, userId });
              }}
            />
            <div>
...
export default function IndexPage() {
  const postsQuery = trpc.message.infiniteMessages.useInfiniteQuery(
    {},
    {
  ...
  const [messages, setMessages] = useState(() => {
    const msgs = postsQuery.data?.pages.map((page) => page.messages).flat();
    return msgs;
  });
  ...
  trpc.message.onNewMessage.useSubscription(undefined, {
    onData(onNewMessage) {
      addMessages([onNewMessage]);
    },
    onError(err) {
      console.error('Subscription error:', err);
      // we might have missed a message - invalidate cache
      utils.message.infiniteMessages.invalidate();
    },
  });
                  ...
                  <article key={item.id} className=" text-gray-50">
                    <header className="flex space-x-2 text-sm">
                      <h3 className="text-base">
                        ...
                        <a
                          href={`https://github.com/${item.message}`}
                          target="_blank"
                          rel="noreferrer"
                        >
                          {item.message}
